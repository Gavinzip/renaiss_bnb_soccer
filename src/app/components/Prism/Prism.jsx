...
const Prism = ({
  ...
  useEffect(() => {
    ...
    const MAX_DPR = Math.max(1, Math.min(2, maxDpr || 2));
    const STEP_COUNT = Math.max(48, Math.min(120, Math.round(steps || 100)));
    ...
    if (suspendWhenOffscreen) {
      const io = new IntersectionObserver(entries => {
        ...
        if (isInViewport && !document.hidden) startRAF();
        else stopRAF();
      });
