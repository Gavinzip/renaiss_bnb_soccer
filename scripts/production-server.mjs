...
async function enrichStoredDrawWinnersSnapshot(payload, networkKey = 'mainnet') {
  const selectedNetworkKey = normalizeDrawNetworkKey(networkKey)
  const enriched = enrichDrawWinnersPayload({
    ...payload,
    networkKey: selectedNetworkKey,
  })
  const withLedgerDownloadState = withDrawWinnersLedgerDownloadState(enriched, selectedNetworkKey)
...
async function readDrawWinnersSnapshot(networkKey = 'mainnet') {
  ...
  if (currentSnapshot.sourceStatus !== 'revealed') {
    return {
      ...await enrichStoredDrawWinnersSnapshot(currentSnapshot, selectedNetworkKey),
      networkKey: selectedNetworkKey,
    }
  }
  ...
  const snapshots = await Promise.all(
    snapshotInputs.map((snapshot) => enrichStoredDrawWinnersSnapshot(snapshot, selectedNetworkKey)),
  )
  ...
  const current = snapshots[snapshots.length - 1] || await enrichStoredDrawWinnersSnapshot(currentSnapshot, selectedNetworkKey)
...
function lockedRoundLedgerFilePath(roundId, networkKey = 'mainnet') {
  const safeRoundId = safeDrawLedgerRoundId(roundId)
  const { lockedRoundsDir } = drawLedgerPathsForNetwork(networkKey)
  return safeRoundId && lockedRoundsDir ? join(lockedRoundsDir, `${safeRoundId}.json`) : ''
}

function drawWinnersLedgerDownloadInfo(payload, networkKey = 'mainnet') {
  const selectedNetworkKey = normalizeDrawNetworkKey(networkKey)
  const roundId = safeDrawLedgerRoundId(payload?.drawRoundId || payload?.draw_round_id || payload?.roundId || payload?.round_id)
  const path = lockedRoundLedgerFilePath(roundId, selectedNetworkKey)
  ...
  return {
    ...
    url: !available
      ...
      : selectedNetworkKey === 'mainnet'
        ? `/match-draw-ledgers/${roundId}.json`
        : `/api/draw-winners/ledger?network=sandbox&roundId=${encodeURIComponent(roundId)}`,
  }
}

function withDrawWinnersLedgerDownloadState(payload, networkKey = 'mainnet') {
  const download = drawWinnersLedgerDownloadInfo(payload, networkKey)
  return {
    ...payload,
    proof: {
...
function drawWinnerProofNetworkKey(payload) {
  return normalizeDrawNetworkKey(payload?.networkKey || payload?.network || 'mainnet')
}
...
const server = createServer(async (request, response) => {
  ...
  if (url.pathname === '/api/draw-winners/ledger') {
    try {
      const networkKey = normalizeDrawNetworkKey(url.searchParams.get('network') || 'mainnet')
      if (!authorizeDrawWinnersNetworkRequest(request, response, networkKey)) return
      const roundId = safeDrawLedgerRoundId(url.searchParams.get('roundId'))
      const ledgerPath = lockedRoundLedgerFilePath(roundId, networkKey)
      if (!roundId || !ledgerPath || !existsSync(ledgerPath) || !statSync(ledgerPath).isFile()) {
        throw Object.assign(new Error('Locked draw ledger is not available for this network and round.'), {
          statusCode: 404,
          code: 'draw_winner_ledger_missing',
        })
      }
      sendJson(request, response, 200, JSON.parse(readFileSync(ledgerPath, 'utf8')), {
        'cache-control': 'no-store',
        'content-disposition': `attachment; filename="${networkKey}-${roundId}-locked-ledger.json"`,
      })
    } catch (error) {
      sendJson(request, response, Number(error?.statusCode || 503), {
        ok: false,
        code: error?.code || 'draw_winner_ledger_failed',
        error: error instanceof Error ? error.message : 'Could not read locked draw ledger.',
      }, { 'cache-control': 'no-store' })
    }
    return
  }
